...
### 1) Replace <REDACTED_PASSWORD> with a value read from an environment variable or secret manager

### 2) Login and store cookie + JSON (placeholder)
...
### 3) Extract SID/CSRF locally (example)
...
### 4) Call an endpoint using cookie + CSRF or X-FTL-SID
...
### 5) Container maintenance examples
